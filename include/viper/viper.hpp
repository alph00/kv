...
static constexpr size_t ONE_GB = 1024l * 1024 * 1024; // B//qfs
...
struct ViperConfig {
  ...
  bool enable_reclamation = false; //注意//qfs
};

namespace internal {

template <typename K, typename V>
constexpr data_offset_size_t
get_num_slots_per_page(const uint32_t entry_size) { // v
  uint16_t current_page_size = PAGE_SIZE;

  const uint16_t page_overhead = sizeof(version_lock_t) + 1; //?//qfs
...
inline void pmem_persist(const void *addr, const size_t len) {
  ...
  for (; addr_ptr < end_ptr; addr_ptr += CACHE_LINE_SIZE) {
    _mm_clwb(addr_ptr); //从cache写到NVM//qfs
  }
  _mm_sfence();
} //写NVM//qfs
...
struct VarSizeEntry_ {
  ...
  VarSizeEntry_(const uint64_t keySize, const uint64_t valueSize,
                uint64_t TID = 0, uint64_t cts = 0, uint64_t prev = 0) // v
      : is_set{true}, key_size{static_cast<uint16_t>(keySize)},
        value_size{static_cast<uint16_t>(valueSize)}, TID{TID}, cts{cts},
        prev_version{prev} {} // to do qfs

}; //变长tuple//qfs

struct VarEntryAccessor_ {
  ...
  VarEntryAccessor_(const char *raw_entry) {
    const VarSizeEntry_ *entry =
        reinterpret_cast<const VarSizeEntry_ *>(raw_entry);
    ...
    key_data = const_cast<char *>(raw_entry) + sizeof(entry->size_info) +
               sizeof(entry->TID) * 3; // v
    value_data = key_data + key_size;
  }

  VarEntryAccessor_(const char *raw_key_entry, const char *raw_value_entry)
      : VarEntryAccessor_{raw_key_entry} {
    assert(value_size == 0);
    assert(key_data != nullptr);
    const VarSizeEntry_ *value_entry =
        reinterpret_cast<const VarSizeEntry_ *>(raw_value_entry);
...
template <typename K, typename V> struct alignas(PAGE_SIZE) ViperPage {
  using VEntry = std::pair<K, V>;
  // v
  using VEntry_ = struct {
    uint64_t TID, cts, prev_version;
    std::pair<K, V> kv;
  };

  static const uint32_t entry_size = sizeof(VEntry_); // v
  static constexpr data_offset_size_t num_slots_per_page =
      get_num_slots_per_page<K, V>(entry_size); // v

  std::atomic<version_lock_t> version_lock;
  std::bitset<num_slots_per_page> free_slots;

  std::array<VEntry_, num_slots_per_page> data;
  ...
  void init() {
    ...
    static_assert(
        PAGE_SIZE % alignof(*this) == 0,
        "VPage not page size conform!"); // why not conform? //qfs//oh,because
                                         // of meta data
    version_lock = USED_BIT;
    free_slots.set(); // all bist are 1//qfs//why occupied all?//qfs
  }
...
template <> struct alignas(PAGE_SIZE) ViperPage<std::string, std::string> {
  uint16_t next_insert_offset; //为什么是两个字节？// qfs?
  std::atomic<version_lock_t> version_lock;
  uint8_t modified_percentage;

  static constexpr size_t METADATA_SIZE = sizeof(version_lock) +
                                          sizeof(next_insert_offset) +
...
struct alignas(PAGE_SIZE) ViperPageBlock {
  ...
  } // block是否被占有，用第一个vpage的vlock来判断?//qfs

  inline bool is_unused() const {
    for (const VPage &v_page : v_pages) { //这里加const干啥?//qfs
      if (IS_BIT_SET(v_page.version_lock, USED_BIT))
        return false;
    }
    return true;
  } //所有vpage都没有被用才是unused//qfs
};  // block//qfs
...
// v
struct ViperInitData_2 {
  int fd;
  ViperFileMetadata *meta;
  std::vector<ViperFileMapping> mappings;
  TxTable_type *TxTable_start;
};
// v

template <typename KeyT> struct KeyAccessor {
  typedef const KeyT *checker_type;
}; //干啥用的?//qfs

template <> struct KeyAccessor<std::string> {
  typedef std::string_view checker_type;
};

template <typename ValueT> struct ValueAccessor {
  typedef ValueT *type;
...
template <> struct ValueAccessor<std::string> {
...
template <typename K, typename V> class Viper {
  using ViperT = Viper<K, V>;
  using VPage = internal::ViperPage<K, V>;
  using KVOffset = KeyValueOffset; // cceh //is what?//qfs
  static constexpr uint64_t v_page_size = sizeof(VPage);
  static_assert(BLOCK_SIZE % v_page_size == 0,
                "Page needs to fit into block."); //最多只能用6个vpage//qfs
...
public:
  ...
  create(const std::string &pool_file, uint64_t initial_pool_size,
         ViperConfig v_config = ViperConfig{});
  static std::unique_ptr<Viper<K, V>>
  open(const std::string &pool_file, ViperConfig v_config = ViperConfig{});
  ...
  public:
    ...
    template <typename UpdateFn> bool update(const K &key, UpdateFn update_fn);
  ...
  // v
  class Txn : public Client {
    friend class Viper<K, V>;

  public:
    bool put_(const K &key, const V &value);

    bool get_(const K &key, V *value);

    template <typename UpdateFn> bool update_(const K &key, UpdateFn update_fn);
  ...
  protected:
    TxTable_type *inner_table_start;
    Txn(ViperT &viper) : Client(viper) {}
    bool put_(const K &key, const V &value, bool delete_old);
    inline bool get_value_from_offset_(KVOffset offset, V *value);

    uint64_t TID;
    uint64_t cts;
  };
  Txn get_txn();
  void commit_txn(Txn *txn);
...
protected:
  // v
  void get_new_access_information_(Txn *txn);
  void get_block_based_access_(Txn *txn);
  void get_new_var_size_access_information_(Txn *txn);
  void recover_database_();
  // v
  ...
  // v
  std::atomic<uint64_t> TxTable_offset;
  using DTT_type = struct {
    uint64_t TID;
    K key;
  };
  std::vector<TxTable_type> TxTable;
  std::vector<DTT_type> DTT;
  std::atomic<size_t> cts;
...
Viper<K, V>::Viper(ViperBase v_base, const std::filesystem::path pool_dir,
    ...
    : v_base_{v_base}, map_{131072}, owns_pool_{owns_pool}, v_config_{v_config},
      pool_dir_{pool_dir}, resize_threshold_{v_config.resize_threshold},
  ...
  inner_table_start = new TxTable_type[TXTABLE_SIZE]; // v
...
template <typename K, typename V> Viper<K, V>::~Viper() {
  if (owns_pool_) {
    DEBUG_LOG("Closing pool file.");
    munmap(v_base_.v_metadata, v_base_.v_metadata->block_offset);
    munmap(v_base_.TxTable_start, PAGE_SIZE); // v
    delete[] inner_table_start;               // v
...
ViperInitData_2 init_devdax_pool(
    const std::string &pool_file, // v 里面有错 TxTable要加上is_new_pool判断
    uint64_t pool_size, bool is_new_pool, ViperConfig v_config,
    const size_t block_size) {
  ...
  MMAP_CHECK(pmem_addr)
  // v
  TxTable_type *TxTable_start_ =
      static_cast<TxTable_type *>(pmem_addr) + PAGE_SIZE; // v
  // v
  ...
  ViperFileMapping initial_mapping{
      .mapped_size =
          alloc_size - metadata->block_offset - metadata->block_offset, // v
      .start_addr = map_start_addr + metadata->block_offset +
                    metadata->block_offset}; // v
  mappings.push_back(initial_mapping);
  block_size_t num_allocated_blocks = initial_mapping.mapped_size / block_size;
  ...
  metadata->num_allocated_blocks = num_allocated_blocks;
  internal::pmem_persist(metadata, sizeof(ViperFileMetadata));
  return ViperInitData_2{
      .fd = fd,
      .meta = metadata,
      .mappings = std::move(mappings),
      .TxTable_start =
          TxTable_start_}; ////
                           /// meta里是最开始的地址，mapping里是去掉meta之后的地址
}

ViperInitData_2 init_file_pool(const std::string &pool_dir, // v
                               uint64_t pool_size, bool is_new_pool,
                               ViperConfig v_config, const size_t block_size) {
  ...
  if (is_new_pool) {
    ...
    ViperFileMetadata v_metadata{.block_offset = PAGE_SIZE,
                                 ...
                                 .total_mapped_size = pool_size};
    internal::pmem_memcpy_persist(metadata_addr, &v_metadata,
                                  sizeof(v_metadata));
    metadata = static_cast<ViperFileMetadata *>(metadata_addr);
  }
  ::close(meta_fd);
  ...
  std::filesystem::path TxTable_file = pool_dir + "/TxTable";
  const int TxTable_fd =
      ::open(TxTable_file.c_str(), VIPER_FILE_OPEN_FLAGS, 0644);
  if (TxTable_fd < 0) {
    IO_ERROR("Cannot open TxTable file: " + TxTable_file.string());
  }
  if (is_new_pool) {
    if (fallocate(TxTable_fd, 0, 0, PAGE_SIZE) != 0) {
      IO_ERROR("Could not allocate: " + TxTable_file.string());
    }
  }

  void *TxTablemem_addr =
      mmap(nullptr, PAGE_SIZE, VIPER_MAP_PROT, VIPER_MAP_FLAGS, TxTable_fd, 0);
  MMAP_CHECK(TxTablemem_addr)
  if (is_new_pool) {
    TxTable_type txtable[TXTABLE_SIZE];
    memset(&txtable, 0x00, PAGE_SIZE);
    internal::pmem_memcpy_persist(TxTablemem_addr, &txtable, sizeof(txtable));
  }
  TxTable_type *TxTable_start_ = static_cast<TxTable_type *>(TxTablemem_addr);
  ::close(TxTable_fd);

  // v
  return ViperInitData_2{.fd = -1,
                         .meta = metadata,
                         .mappings = std::move(mappings),
                         .TxTable_start = TxTable_start_}; // v
}
...
ViperBase Viper<K, V>::init_pool(const std::string &pool_file,
  ...
  return ViperBase{.file_descriptor = init_data.fd,
                   ...
                   .TxTable_start = init_data.TxTable_start}; // v
}
...
template <typename K, typename V> void Viper<K, V>::recover_database() {
    ...
    for (block_size_t block_num = start_block; block_num < end_block;
      ...
      for (page_size_t page_num = 0; page_num < num_pages_per_block;
        ...
        for (data_offset_size_t slot_num = 0;
          ...
          // Data is present
          const K &key = page.data[slot_num].kv.first; // v
...
template <> void Viper<std::string, std::string>::recover_database() {
  // TODO
...
template <typename K, typename V> KeyValueOffset Viper<K, V>::get_new_block() {
...
template <typename K, typename V> void Viper<K, V>::trigger_resize() {
...
bool Viper<K, V>::Client::put(const K &key, const V &value,
  ...
  typename VPage::VEntry_ *entry_ptr = v_page_->data.data() + free_slot_idx;
  internal::pmem_persist(entry_ptr, sizeof(typename VPage::VEntry_));
...
bool Viper<std::string, std::string>::Client::put(const std::string &key,
  ...
  if (offset_in_page + meta_size + entry_length > v_page_size) {
    ...
    if (offset_in_page + meta_size + entry.key_size <= v_page_size &&
      ...
      current_v_page = v_page_; // current_v_page干啥用的//?qfs
      is_inserted = true;
    } else {
      ...
      if (offset_in_page + meta_size <= v_page_size) {
        ...
        internal::pmem_memcpy_persist(insert_pos, &next_page_entry.size_info,
                                      meta_size);
      }
...
template <typename K, typename V>
inline void
Viper<K, V>::Client::invalidate_record(VPage *v_page,
                                       const data_offset_size_t data_offset) {
  if constexpr (std::is_same_v<K, std::string>) {
    char *raw_data = &v_page->data[data_offset];
    internal::VarSizeEntry *var_entry =
        reinterpret_cast<internal::VarSizeEntry *>(
            raw_data); // to do qfs 这里应该得改
...
template <typename K, typename V> Viper<K, V>::Client::~Client() {
...
Viper<K, V>::ReadOnlyClient::get_const_entry_from_offset(
  ...
  if constexpr (std::is_same_v<K, std::string>) {
    ...
    internal::VarEntryAccessor_ var_entry{raw_data}; // v
    if (var_entry.value_size == 0) {
      // Value is on next page
      const char *raw_value_data = &v_block->v_pages[page + 1].data[0];
      var_entry = internal::VarEntryAccessor_{raw_data, raw_value_data}; // v
    }
...
template <typename K, typename V>
inline bool
Viper<K, V>::ReadOnlyClient::get_const_value_from_offset(KVOffset offset,
                                                         V *value) const {
  const auto [block, page, slot] = offset.get_offsets();
...
inline bool Viper<K, V>::Client::get_value_from_offset(const KVOffset offset,
  ...
  *value = v_page.data[slot].kv.second; // v
  return lock_val == page_lock.load(LOAD_ORDER);
}
...
void Viper<K, V>::compact(Client &client, VPageBlock *v_block) {
  for (VPage &v_page : v_block->v_pages) {
    ...
    for (size_t slot = 0; slot < v_page.num_slots_per_page; ++slot) {
      ...
      const auto &record = v_page.data[slot].kv; // v
...
template <typename K, typename V> void Viper<K, V>::reclaim() {
...
template <> void Viper<std::string, std::string>::reclaim() {
  const block_size_t max_block =
      KVOffset{current_block_page_.load(LOAD_ORDER)}.block_number;
...
// v
template <typename K, typename V>
inline typename Viper<K, V>::Txn Viper<K, V>::get_txn() {
  num_active_clients_++;
  //
  Txn txn{*this};
  txn.inner_table_start = inner_table_start; // v

  //获得TID
  txn.TID = Viper::TID.fetch_add(1) + 1; //避开0
  //持久化事务表项,
  int64_t offset = -1;
  while (offset == -1) {
    for (int i = 0; i < TXTABLE_SIZE; ++i) {
      TxTable_type *off = v_base_.TxTable_start + i;
      uint64_t tid = txn.TID;
      bool state = true;
      uint64_t zero = 0;
      if (off->TID.compare_exchange_strong(zero, tid)) {
        offset = i;
        break;

      } else if (off->state.compare_exchange_strong(state, false)) {
        off->TID.store(txn.TID);
        offset = i;
        break;
      }
    }
  ...
  internal::pmem_persist(v_base_.TxTable_start + offset, sizeof(TxTable_type));

  // TxTable_type *a = new TxTable_type[260];
  // memcpy(a, v_base_.TxTable_start, PAGE_SIZE); // v
  // for (int i = 0; i < TXTABLE_SIZE - 1; ++i) {
  //   std::cout << i << " "
  //             << "OFFSET" << (a + i)->state << " "
  //             << "TID" << (a + i)->TID << "\n";
  // }
  ...
  if constexpr (std::is_same_v<K, std::string>) {
    get_new_var_size_access_information_(&txn); // to do
  } else {
    get_new_access_information_(
        &txn); //当且仅当get_client和put的时候触发GC(以info_sync的形式）//qfs//这个函数就是GC一下，重写一下metadata
  }
  return txn;
}
template <typename K, typename V>
void Viper<K, V>::commit_txn(Viper::Txn *txn) {
  //领取提交时间戳
  txn->cts = Viper::cts.fetch_add(1);
  //持久化事务表项
  int64_t offset = -1; //肯定能找到entry，故-1不影响
  for (int i = 0; i < TXTABLE_SIZE; ++i) {
    TxTable_type *off = v_base_.TxTable_start + i;
    if (txn->TID == off->TID.load(std::memory_order_relaxed)) {
      off->state.store(true);
      offset = i;
      break;
    }
  }
  ...
  // }
  // delete[] a;
  txn->Client::info_sync(true);
  --num_active_clients_;
}

template <typename K, typename V>
void Viper<K, V>::get_new_var_size_access_information_(Txn *txn) {
  if constexpr (!std::is_same_v<K, std::string>) {
    throw std::runtime_error("Cannot update var pages for fixed-size entries.");
  }

  get_new_access_information(txn);
  txn->v_page_->next_insert_offset = 0;
...
template <typename K, typename V>
void Viper<K, V>::get_new_access_information_(Txn *txn) {
  // Get insert/delete count info
  txn->Client::info_sync(true);
  ...
  get_block_based_access_(txn);
}

template <typename K, typename V>
void Viper<K, V>::get_block_based_access_(Txn *txn) {
  block_size_t client_block = -1;
  page_size_t client_page = 0;
  if (!free_blocks_.try_dequeue(client_block)) {
    // No free block available, get new one.
  ...
  txn->Client::strategy_ = Client::PageStrategy::BlockBased;
  txn->Client::v_block_number_ = client_block;
  txn->Client::v_page_number_ = client_page;
  txn->Client::num_v_pages_processed_ = 0;
  ...
  if (txn->Client::v_block_ != nullptr) {
    txn->Client::v_block_->v_pages[0].version_lock &= NO_CLIENT_BIT;
  }

  txn->Client::v_block_ = v_blocks_[client_block];
  txn->Client::v_page_ = &(txn->Client::v_block_->v_pages[client_page]);
  txn->Client::v_page_->init();
  txn->Client::v_block_->v_pages[0].version_lock |= CLIENT_BIT;
...
template <typename K, typename V>
bool Viper<K, V>::Txn::put_(const K &key, const V &value,
                            const bool delete_old) {
  Client::v_page_->lock();

  // We now have the lock on this page
  std::bitset<VPage::num_slots_per_page> *free_slots =
      &Client::v_page_->free_slots;
  const data_offset_size_t free_slot_idx = free_slots->_Find_first();

  if (free_slot_idx >= free_slots->size()) {
    // Page is full. Free lock on page and restart.
    Client::v_page_->unlock();
    Client::update_access_information();
    return put_(key, value, delete_old);
  }

  // We have found a free slot on this page. Persist data.
  Client::v_page_->data[free_slot_idx].kv = {key, value};
  Client::v_page_->data[free_slot_idx].TID = this->TID; // v
  typename VPage::VEntry_ *entry_ptr =
      Client::v_page_->data.data() + free_slot_idx;
  internal::pmem_persist(entry_ptr, sizeof(typename VPage::VEntry_));
  ...
  // Store data in DRAM map.
  const KVOffset kv_offset{Client::v_block_number_, Client::v_page_number_,
                           free_slot_idx};
  KVOffset old_offset;
  ...
  if (!is_new_item && delete_old) {
    // Need to free slot at old location for this key
    Client::free_occupied_slot(old_offset, key);
  }
...
template <typename K, typename V>
bool Viper<K, V>::Txn::put_(const K &key, const V &value) {
  return put_(key, value, true);
}

template <typename K, typename V>
bool Viper<K, V>::Txn::get_(const K &key, V *value) {
  auto key_check_fn = [&](auto key, auto offset) {
    if constexpr (using_fp) {
      return this->viper_.check_key_equality(key, offset);
    } else {
  ...
  while (true) {
    ...
    if (get_value_from_offset_(kv_offset, value)) {
      return true;
    }
  }
}
...
template <typename K, typename V>
inline bool Viper<K, V>::Txn::get_value_from_offset_(const KVOffset offset,
                                                     V *value) {
  const auto [block, page, slot] = offset.get_offsets();
  const VPage &v_page = this->viper_.v_blocks_[block]->v_pages[page];
  const std::atomic<version_lock_t> &page_lock = v_page.version_lock;
  ...
  for (int i = 0; i < TXTABLE_SIZE; ++i) {
    ...
    TxTable_type *off = this->inner_table_start + i;
    // if (off->state == 1)
    // std::cout << i << ' ' << "STATE" << off->state << "TID" << off->TID <<
    // '\n';
    if (v_page.data[slot].TID == off->TID && off->state == false) {
      return false;
    } // to do
  }
  // v
...
template <typename K, typename V> void Viper<K, V>::recover_database_() {
  auto start = std::chrono::steady_clock::now();

  // v
  memcpy(inner_table_start, v_base_.TxTable_start, PAGE_SIZE); // v
  // for (int i = 0; i < TXTABLE_SIZE; ++i) {
  //   std::cout << "i: " << i << "OFFSET " << (inner_table_start + i)->state
  //             << "TID " << (inner_table_start + i)->TID << "\n";
  // }
  // v
...
template <typename K, typename V> Viper<K, V>::Txn::~Txn() {
  // this->viper_.remove_client(this);有以下两行组成
  // client->info_sync(true);//感觉没必要
  //--Client::num_active_clients_;//这个不是原子变量吗？好奇怪啊//这个变量实际上没有用到，暂时先不加
...
// TID跟着key走
template <>
bool Viper<std::string, std::string>::Txn::put_(const std::string &key,
                                                const std::string &value,
                                                const bool delete_old) {
  v_page_->lock();
  VPage *start_v_page = v_page_;

  internal::VarSizeEntry_ entry{key.size(), value.size(), this->TID}; // v
  bool is_inserted = false;
  char *insert_pos = v_page_->data.data() + v_page_->next_insert_offset;
  const size_t entry_length = entry.key_size + entry.value_size;
  const size_t meta_size = sizeof(entry.size_info);
  ...
  // v
  const size_t Tx_Item_size = sizeof(entry.TID);
  const size_t Tx_size =
      sizeof(entry.TID) * 3; // to do qfs//这里可能会在不同字长的机器里出问题
  // v

  if (offset_in_page + Tx_size + meta_size + entry_length > v_page_size) { // v
    ...
    if (offset_in_page + meta_size + Tx_size + entry.key_size <= v_page_size &&
        !force_new_page) { // v
      // Key fits, value does not. Add key to old page and value to new one.
      char *key_insert_pos = insert_pos;
      ...
      // 0 size indicates key is on previous page.
      insert_pos = v_page_->data.data();
      internal::VarSizeEntry_ value_entry{0, value.size()};
      value_entry.data = insert_pos + meta_size;
      memcpy(value_entry.data, value.data(), value_entry.value_size);
      memcpy(insert_pos, &value_entry.size_info, meta_size);
      internal::pmem_persist(insert_pos, meta_size + value_entry.value_size);
      ...
      // 0 size indicates value is on next page.
      entry.value_size = 0;
      entry.data = key_insert_pos + meta_size + Tx_size; // v
      memcpy(entry.data, key.data(), entry.key_size);
      memcpy(key_insert_pos, &entry.size_info, meta_size);
      // v
      memcpy(key_insert_pos + meta_size, &entry.TID,
             Tx_size); // to do qfs //不好说可不可以一个memcpy写三个量
      // v
      internal::pmem_persist(key_insert_pos,
                             meta_size + entry.key_size + Tx_size);

      current_v_page->next_insert_offset = VPage::DATA_SIZE; //?qfs
      internal::pmem_persist(current_v_page, insert_offset_size);
      v_page_->next_insert_offset +=
          meta_size + value_entry.value_size; //不用加Tx_size
      internal::pmem_persist(v_page_, insert_offset_size);
      current_v_page = v_page_;
      is_inserted = true;
    } else {
      ...
      if (offset_in_page + meta_size <= v_page_size) {
        // 0-entry metadata fits into current page.
        internal::VarSizeEntry_ next_page_entry{0, 0};
        internal::pmem_memcpy_persist(insert_pos, &next_page_entry.size_info,
                                      meta_size);
      }
  ...
  if (!is_inserted) {
    ...
    entry.data = insert_pos + meta_size + Tx_size; // v
    memcpy(insert_pos, &entry.size_info,
           meta_size); // meta_size写了两遍？//?qfs

    // v
    memcpy(insert_pos + meta_size, &entry.TID, Tx_Item_size);
    memcpy(insert_pos + meta_size + Tx_Item_size, &entry.cts, Tx_Item_size);
    memcpy(insert_pos + meta_size + Tx_Item_size * 2, &entry.prev_version,
           Tx_Item_size);
    // v

    memcpy(entry.data, key.data(), entry.key_size);
    memcpy(entry.data + entry.key_size, value.data(), entry.value_size);
    internal::pmem_persist(insert_pos,
                           meta_size + entry_length + Tx_size);        // v
    v_page_->next_insert_offset += meta_size + entry_length + Tx_size; // v
    internal::pmem_persist(v_page_, insert_offset_size);
    _mm_prefetch(v_page_, _MM_HINT_T0); //?qfs
  }
  ...
  old_offset = this->viper_.map_.Insert(key, var_offset, key_check_fn); // v
  // KVOffset t = this->viper_.map_.Get(key, key_check_fn);                //
  // std::cout << t.is_tombstone() << " " << key << "\n";                  //
  // note
  is_new_item = old_offset.is_tombstone();
  size_delta_++;
...
template <>
inline bool Viper<std::string, std::string>::Txn::get_value_from_offset_(
    const KVOffset offset, std::string *value) {
  const auto [block, page, data_offset] = offset.get_offsets();
  const VPageBlock *v_block = this->viper_.v_blocks_[block];
  const VPage &v_page = v_block->v_pages[page];
...
template <> void Viper<std::string, std::string>::recover_database_() {
  // TODO
  throw std::runtime_error("Not implemented yet");
}
